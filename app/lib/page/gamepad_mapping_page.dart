...
class _GamepadMappingPageState extends State<GamepadMappingPage> {
  ...
  String _editingKey = ""; // The key currently being edited
  ...
  void _loadMappings() async {
    ...
    if (mappingStr != null) {
      try {
        ...
        // Use default mapping if loading fails
        _loadDefaultMappings();
      }
    } else {
      // If no saved mapping, use default mapping
      _loadDefaultMappings();
    }
  }
  ...
  void _startListening() {
    ...
    _subscription = Gamepads.events.listen((event) async {
      ...
      // Get the most frequently detected value
      double maxValue = 0;
      int maxCount = 0;
      ...
      if (maxCount >= detect_count) {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Mapping successful. Key mapped to \${maxResult!.key}')),
          );
        }
      ...
      // If currently mapping, record detected value
      ...
      _msg =
          'Push the stick or press the key to this position multiple times. Detected: ${_recvEvent.key}: ${_recvEvent.value.toStringAsFixed(2)}, best: ${maxResult?.key}: ${maxResult?.value.toStringAsFixed(2)}, triggers remaining: ${detect_count - maxCount}';
      setState(() {});
    });
  }
