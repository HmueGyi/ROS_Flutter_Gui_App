...
class OccupancyMap {
  ...
  List<int> getCostMapData() {
    ...
    for (int x = 0; x < mapConfig.height; x++) {
      for (int y = 0; y < mapConfig.width; y++) {
        ...
        // Inscribed obstacle: the robot's inscribed radius zone; entering this zone guarantees a collision
        if(pixelValue==99){
        ...
        // Add the RGBA values to the result array
  ...
  /**
   * @description: Given grid map coordinates, returns the global (world) coordinates at that position
   * @return {*}
   */
  ...
  /**
   * @description: Given global (world) coordinates, returns the grid map row and column index
   * @return {*}
   */
  ...
  void saveMap(String path) {
    ...
    // Write to file: write text header first, then binary data
  ...
  // Convenience save method: specify directory and filename
